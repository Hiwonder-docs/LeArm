...
:::{Note}
* All servos on the LeArm robotic arm are factory-calibrated to their center positions, so no additional setup is required.
* For details on the communication protocol of the bus servo, please refer to the [Bus Servo Control](5.Basic_Development_Course.md#serial-bus-servo-instructions) folder within the corresponding main controller programming course.
:::
